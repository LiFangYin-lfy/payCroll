  ...
  <div class="container">
    <div class="paycroll1">
      ...
      <div class="title_name">
        <div class="han_title">五险一金及个税计算器-税前工资</div>
        <div class="Eng_title">
          Personal Income Tax Calculator (Before-tax algorithm)
        </div>
      </div>
    </div>
    <div class="payroll2">
    ...
    <div class="btn_sure">
      <van-button round size="large" color="#3886EF" @click="onSubmit">
        计算 Calculate
      </van-button>
    </div>
  </div>
...
export default {
  ...
  methods: {
      ...
      if (obj.salary == '') {
        ...
        try {
          ...
          if (data.code == 1) {
            ...
            that.$router.push({ path: '/payrollB', query: { result: JSON.stringify(result) } });
          } else {
            Toast.fail(data.msg)
          }
        } catch (err) {
    ...
    salaryHandAverage() {
      console.log(this.submit.salary);
      let art = Number(this.submit.salary)
      ...
      console.log(art, social_bottom, housing_fund_bottom);
      if (this.switchTrue == true) {
        if (art < social_bottom) {
          this.submit.social_base = social_bottom
          console.log(this.submit.social_base, "10");

        } else if (art > social_top) {
          this.submit.social_base = social_top
          console.log(this.submit.social_base, "11");

        } else {
          this.submit.social_base = this.submit.salary
          console.log(this.submit.social_base, "12");
        }
      }
      ...
      if (this.switchTap == true) {
        ...
        if (art < housing_fund_bottom) {
          ...
          console.log(this.submit.housing_fund_base, "1");
        } else if (art > housing_fund_top) {
          this.submit.housing_fund_base = housing_fund_top
          console.log(this.submit.housing_fund_base, "2");
        } else {
          this.submit.housing_fund_base = art
          console.log(this.submit.housing_fund_base, "3");
        }
      }
    },
    changeSwitchFalse() {
